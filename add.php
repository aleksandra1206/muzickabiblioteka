    ...
    if (empty($_POST['year'])) {
        ...
        if (strlen($yearStr) != 4 && intval($yearStr) == 1) {
            ...
            if ($year < 1860 || $year > date("Y")) {
                $errors['year'] = 'Enter a year between 1860 and the current year!';
            }
        }
    }
...
<section class="container">
    ...
    <form action="<?php echo $_SERVER['PHP_SELF'] ?>" class="white form" method="POST">
        ...
        <div class="center">
            <input type="submit" name="add" value="Post a song" class="btn pink darken-2 z-depth-0">
        </div>
    </form>
...
<script>
    function showSuggestion(str = "") {
        if (str.length == 0) {
            ...
            xmlhttp.open("GET", "ajax/countries.php?qc=" + str); 
            xmlhttp.send(); 
        }
    }
</script>
...
<script>
    function suggestGenre(str = "") {
        if (str.length == 0) {
            ...
            xmlhttp.open("GET", "ajax/genre.php?query=" + str);
            xmlhttp.send();
        }
    }
</script>
...
<script>
    function suggestArtist(str = "") {
        if (str.length == 0) {
            ...
            xmlhttp.open("GET", "ajax/artists.php?query=" + str);
            xmlhttp.send();
        }
    }
</script>
